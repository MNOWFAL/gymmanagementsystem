...
def registration(request):
    if request.method == "POST":
        ...
        user = User.objects.create_user(
            first_name=fname, last_name=lname, email=email, password=pwd, username=email
        )
...
def editPackage(request, pid):
    ...
    if request.method == "POST":
        ...
        Package.objects.filter(id=pid).update(
            category=categoryid,
            packagename=packageobj,
            titlename=titlename,
            packageduration=duration,
...
def bookingReport(request):
    ...
    if request.method == "POST":
        ...
        data = Booking.objects.filter(
            creationdate__gte=fromdate, creationdate__lte=todate
        )
        data2 = True
...
def apply_booking(request, pid):
    ...
    booking = Booking.objects.create(
        package=data, register=register, bookingnumber=random_with_N_digits(10)
    )
